...
def issue_body(item: dict[str, Any], queue_hash: str) -> str:
    actions = "\n".join(f"- [ ] `{action}`" for action in item.get("discovery_actions", []))
    blockers = "\n".join(f"- `{blocker}`" for blocker in item.get("blockers", [])) or "- none"
    return f"""## Machine-owned state

- Provider: `{item['dependency_id']}`
- Service: `{item['service_name']}`
- Queue state: `{item['inventory_state']}`
...
def sync(repo: str, item: dict[str, Any], queue_hash: str) -> dict[str, Any]:
    provider_id = item["dependency_id"]
    title = issue_title(provider_id)
    ...
    if number:
        ...
        action = "created"
    return {"provider_id": provider_id, "action": action, "issue": number}
...
def main() -> int:
    ...
    queue = load_queue()
    queue_hash = str(queue.get("queue_hash", ""))
    plan = {
    ...
    for item in queue["tasks"]:
        ...
        if args.apply:
            entry["result"] = sync(args.repo, item, queue_hash)
